...
processCoord _ _ [] = []
processCoord gridCoords c (o:os) = o : c : an1 ++ an2 ++ n ++ p
  where
...
antinode (x1,y1) (x2,y2) (gx,gy) | newX >= 0 && newX < gx && newY >= 0 && newY < gy = (newX,newY) : antinode (x2,y2) (newX,newY) (gx,gy)
                                  | otherwise = []
  where
    xDiff = x2 - x1
    yDiff = y2 - y1
